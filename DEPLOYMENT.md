...
Pushes to `main` trigger GitHub Actions. Media (`build/_shared/`, `build/_assets/`, each
language's `build/V. 1.0/<lang>/_images/` and `_downloads/`, and — when a full build
produced one — `build/Offline manual.zip`) syncs to an **AWS S3** bucket fronted by
**CloudFront**. HTML is baked into a tiny nginx Docker image pushed to **GHCR**
(`ghcr.io/flexibowl/flexibowl-docs:latest`). A webhook notifies the Makai Labs Dokploy
server to pull the new image and restart. Live site: `flexibowl-docs.flexibowl.com`.
...
A `--mode full` build (see `README.md` → Build) produces `build/Offline manual.zip`, an
archive of the entire site — including media — written to the root of `build/` by
`build_manual.bat`. A `quick` build doesn't produce it, and that's the normal case, so
most pushes have no archive to handle at all.

ARS doesn't have to do anything different — `git push` behaves the same either way.
Behind the scenes, when the archive is present CI uploads it to the CDN like any other
offloaded file, and the deployment's nginx image has a hard-coded `302` redirect from
`/Offline manual.zip` to the CDN copy, so the "Download offline manual" button (built by
JavaScript at page load against the docs domain, and so unreachable by CI's HTML
rewriter) streams from CloudFront like everything else. The redirect target is baked
into `nginx.conf` from the `CDN_BASE_URL` secret at image-build time.

If `build_manual.bat` is ever changed to rename or relocate this archive, ping Makai
Labs so the redirect, the `.dockerignore` rule, and the LFS-guard allowlist can be
updated together.
...
- `build/Offline manual.zip` (from a full build) archives the entire site, so it's on
  the order of the whole `build/` tree — currently ~2.7 GB. It's LFS-tracked (`*.zip` in
  `.gitattributes`), which is what keeps a push producing one from being rejected by
  GitHub's 100 MB hard limit or blocked by the size-check workflow.
