...
Once you receive your FlexiVision One kit, verify that all the items listed below are included and in good condition.
...
* - 1
  - Camera
* - 1
  - Lens *(focal length specific to the selected working distance)*
* - 1
...
```{note}
The calibration grid and lens may differ from the images depending on the selected kit.
```
...
---
```{important}
If the received items do not match the order or appear damaged, **do not sign the delivery document**. Contact FlexiVision One support or your local distributor immediately.
```
